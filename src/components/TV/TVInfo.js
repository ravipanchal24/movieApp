      ...
      <div className="text-black bg-violet-400 pt-[3rem]">
        <div className="flex items-center justify-center gap-16 py-8 px-20 ">
          {TvInfo.poster_path ? (
            ...
            <div className="w-[250px] h-[350px] rounded-md border border-black flex items-center justify-center my-4 mx-auto">
              <p>Poster Not Available</p>
            </div>
          )}
          <div className="w-[70%]">
            <h1 className="text-3xl my-4">{TvInfo.name}</h1>
            <h2 className="mb-4">
              <b>Genre:</b>{" "}
              {TvInfo.genres
                ? TvInfo.genres.map((item, index) => (
            ...
            <h2 className="mb-8">
              <b>Release Date</b> :{" "}
              {TvInfo.first_air_date ? TvInfo.first_air_date : "NA"}
            </h2>
            <p className="mb-2">
              <b>Overview</b>
            </p>
            <p className="mb-8">{TvInfo.overview ? TvInfo.overview : "NA"}</p>
            <p className="mb-2">
              <b>No. of Seasons</b> :{" "}
              {TvInfo.number_of_seasons ? TvInfo.number_of_seasons : "NA"}
            </p>
            <p className="mb-2">
              <b>No. of Episodes</b> :{" "}
              {TvInfo.number_of_episodes ? TvInfo.number_of_episodes : "NA"}
            </p>
            <p className="mb-4">
              <b>Cast</b> :{" "}
              {cast
                ? cast?.cast.map((item, index) => (
                    <span key={index}> {item.original_name} |</span>
                  ))
                : "NA"}
            </p>
        ...
        <div className="flex items-center justify-center gap-24 flex-wrap">
          {similarTvShows?.results?.map((item, index) => (
            <div className="" key={index}>
              <h2 className="text-center text-2xl w-[370px] h-[2.2rem] overflow-hidden whitespace-nowrap text-ellipsis">
                {item.name}
              </h2>
              <Link to={`/tv/${item.id}`}>
                {item.poster_path ? (
